...
function explore(obj, pth, start) {
  ...
  for (var i = 0; i < cont.length; i++) {
    ...
    if (isDir(newPath)) {
      ...
      var file = cont[i].match(/[^\\]*\.(\w+)$/); // Separate file and extension
      ...
      var b = path.basename(cont[i], ".js"); // Base name of the file

      // Require the JS file add it to the final object
      addPathToObj(obj, pth, start)[b] = require(path.join(pth, b));
    }
  }
}
