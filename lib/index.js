 ...
 * @param {String}  opts.extension  the extension to find
...
module.exports = function requireDirObject(startPath, opts) {
  ...
  function addPathToObj(filePath) {
    ...
    var dirs = filePath.split(path.sep).map(function (j) {
      return convertCase(path.basename(j, extension), opts.case);
    });
  ...
  function explore(pth, depth) {
    ...
    if (opts.depth !== undefined && depth > parseInt(opts.depth, 10)) {
      return;
    }

    if (depth > 0 && cont.indexOf("index" + extension) >= 0) {
      ...
      cont.forEach(function(item) {
        ...
        if (isDir(newPath)) {
          ...
          if (file == null || file[1] !== extension.substring(1)) {
            return;
          }

          // Base name of the file
          var formattedKey = convertCase(path.basename(item, extension), opts.case);

          // Require the JS file add it to the final object
          addPathToObj(pth)[formattedKey] = require(newPath);
        }
      });
    }
  }
